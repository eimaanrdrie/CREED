...
  Uses Qwen models running locally hosted through Ollama for issue understanding, evidence-backed investigation and governed self-learning.
...
DATABASE\_URL=postgresql+psycopg://creed\:creed\@postgres:5432/creed

OLLAMA\_BASE\_URL=http\://host.docker.internal:11434

OLLAMA\_MODEL=qwen3.5:9b

OLLAMA\_RUNTIME\_MODEL=qwen3.5:4b

OLLAMA\_INVESTIGATION\_MODEL=qwen3.5:4b

OLLAMA\_LEARNING\_MODEL=qwen3.5:4b

EMBEDDING\_PROVIDER=ollama

EMBEDDING\_MODEL=qwen3-embedding:0.6b

DOCUMENT\_STORAGE\_PATH=.data/documents

LANGGRAPH\_CHECKPOINT\_PATH=.data/langgraph-checkpoints.sqlite
...
\`DEMO\_MODE\_ENABLED=true\` is intended for judging or rehearsal only. Keep it disabled in a normal environment.
...
\| \`creed\_postgres\` | PostgreSQL data |

\| \`creed\_documents\` | Original evidence files |

\| \`creed\_langgraph\` | LangGraph checkpoints |
...
**## Agentic AI, Local GEN AI and Zero Data Egress**
...
In the documented local deployment, Qwen inference runs through Ollama, embeddings are generated locally, evidence is retrieved from the local repository and pgvector store, and governed learning is processed inside the CREED environment.
...
No external AI provider is required for this flow. Zero Data Egress assumes CREED is operated with the documented local services and without enabling external integrations.
...
\| Release integration | \`python scripts/verify\_r94\_0\_6\_m08.py\` |

\| Current UI verifier | \`python scripts/verify\_ui\_r99\_m03.py\` |
